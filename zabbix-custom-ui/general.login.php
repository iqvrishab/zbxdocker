...
$guest = $data['guest_login_url']
    ? (new CDiv(
        (new CLink(_('sign in as guest'), $data['guest_login_url']))
            ->addClass('btn btn-primary btn-block mt-2 signin-guest')  // add custom class
            ->addStyle('width: 100%; display: block; text-align: center;') // enforce full width
    ))
    : null;
